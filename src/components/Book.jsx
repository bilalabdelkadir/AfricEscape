...
const Book = ({ id, token }) => {
  ...
  let {mutate, status, isLoading, error} = useMutation(() => bookTour(id, token), {
    onSuccess: (res) => {
...
console.log(error, 'the error')
  ...
  return (
    <div className="w-[90%] px-10 md:w-[80%] h-[10rem] new-shadow text-primary mt-10 rounded-xl flex flex-col items-center justify-center md:justify-around mx-auto md:items-center">
      ...
      <button
        onClick={bookHandler}
        className=" hover:bg-green-600 bg-gray-800 bg-primary w-3/4 md:w-1/4 text-white rounded-2xl py-2 px-5"
      >
        {/* react query mutation isLoading used  */}
        { !isLoading ? <span className="text-white">Book Now</span> : <Spinner /> }
      </button>
      {status === 'idle' && <></>}
      {status === 'success' && (
